...
class AirlinesController {
  ...
  async getAirlineByCountry(req, res) {
    ...
    try {
      const Airline =
        await airlinesServices.getAirlineByCountry(AirlineCountry);
      if (Airline) {
        return res.status(200).json({
          ok: true,
          datos: Airline,
  ...
  async updateAirline(req, res) {
    ...
    try {
      const numFilas = await airlinesServices.updateAirline(
        id_airline,
        airline,
      );
      ...
      if (numFilas == 0) {
        return res.status(404).json({
          ok: false,
          datos: null,
          mensaje: "No encontrado o sin cambios",
        });
      } else {
        // CAMBIA ESTO: En lugar de 204, enviamos un 200 con el objeto ok: true
        return res.status(200).json({
          ok: true,
      ...
      return res.status(500).json({
        ok: false,
        datos: null,
        mensaje: "Error al actualizar aerolínea",
      });
    }
  }
}
